...
global.play = async (queue, message) => {
  if (queue[0] == undefined) return message.channel.send('В очереди ничего нет :no_entry_sign:')
  if (!message.member.voice.channel) return message.channel.send('Вы не в канале :no_entry_sign:')
